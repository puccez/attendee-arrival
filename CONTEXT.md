...
- **Check-in** — la presenza registrata di un attendee a un evento, etichettata su due assi ortogonali (non una scala):
  ...
  - **Qualità** (quanto è solida, continua): quanti codici raccolti e su che arco di tempo (dwell), tap sulla notifica, coerenza dei segnali.
- **Dwell opportunistico** — la permanenza misurata dai codici raccolti nell'arco della serata. Non è campionamento continuo (iOS non lo consente in background): si accumula quando il telefono può ascoltare. È anche la difesa principale anti-relay: inoltrare un codice costa un messaggio, inoltrarne per due ore costa un complice dedicato.
