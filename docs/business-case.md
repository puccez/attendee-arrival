...
| **Non-negoziabile: fermare i check-in falsi** | Il GPS non è mai una prova. La prova è aver ricevuto un segreto effimero emesso *al venue, in quel minuto*. Le quattro frodi del brief, più il replay tardivo, sono eseguibili dal vivo | **Sandbox d'attacco**: ogni attacco parte davvero e atterra in dashboard con la sua etichetta (sezione 6) |
| Segnali che distinguono posizione genuina da spoofata | Sezione dedicata: usati come contesto e come telemetria antifrode, mai come accreditamento | sezione 6.6 |
| Cosa succede se l'attendee ignora la notifica | Cascata dichiarata: tap → nessun tap ma codici → nessun codice ma Arrivo → prompt all'host | sezione 7.2 |
| Come l'host vede lo stato in tempo reale | Dashboard con provenienza × qualità per ogni riga, battito del beacon, testimonianza umana a un tap | Console dell'evento (sezione 5) |
| Edge case: back-to-back, niente rete, check-in di gruppo | Seme per-evento; borsellino offline con prova autodatante; rifiuto esplicito del «garantisco io per i miei amici» | sezione 8 |
...
consegna offline: il conto è nella sezione 6.3.
...
- **macchina**: codici consegnati e verificati. A rigore prova che quel
  ...
  al prezzo di un complice presente (sezione 6.3).
...
| macchina | 1 codice, 0 min | C'era. Non è rimasto | Presenza sì, ricompense legate alla permanenza no (sezione 6.4) |
| umano | – | L'host ha visto la persona. Nessuna prova del device | Presenza sì; il rischio è la frode sociale, dichiarata (sezione 9) |
...
| **Benefici con valore reale** (referral, selezione host) | `macchina + umano`; la biometria al tap (hardening sezione 12) si *aggiunge* contro il prestito del telefono, non sostituisce lo sguardo dell'host, perché lega la persona al device, non il device al venue |
| **Solo dichiarazioni** (GPS, tap) | mai un accreditamento |

Due onestà a corredo. La prima: il ritardo di consegna alto non distingue
l'offline onesto dall'inoltro differito (sezione 6.3): per gli usi che contano, la
...
campo guardando le distribuzioni reali di copertura e buchi (sezione 11). Quello che
...
- **l'inoltro informato**: chi riceve dichiara l'ora di *nascita* del codice,
  ...
  default). È la stessa proprietà che fa funzionare l'offline (sezione 8.2): il server
...
Il dwell (sezione 6.4) resta la difesa strutturale: un check-in sostenuto da un solo
...
è «rimasto abbastanza»: esistono soglie per uso (sezione 4), e la differenza resta
...
esiste perché l'offline è normale (sezione 8.2); il margine di replay che introduce è
esattamente quello messo a nudo nella sezione 6.3, finito, misurato dal ritardo di
...
1. **Tap sulla notifica** → il check-in che i codici stanno già producendo si
   arricchisce di `tap ✓` nella qualità (il tap da solo non crea nulla: sezione 6.2).
...
4. **Niente di niente** (telefono scarico, permessi negati, nessuna app) → l'host
   ...
   d'ufficio richiede il join con gli iscritti, dichiarato nella sezione 9.2.)
...
profondità, vedi la sezione 9.2.)
...
miei tre amici»* **è** una delle frodi che il brief chiede di fermare (sezione 6.2), solo
...
- **Chi non può, passa dall'host.** Un telefono scarico nel gruppo è la casistica
  della sezione 7.2 gradino 4: l'host testimonia quella persona, con provenienza `umano`.
...
L'evoluzione naturale è la **testimonianza tra pari** (sezione 12): i telefoni si rilevano
...
del rollout (sezione 11) misura i flussi ottici e i passaggi dall'host; i risvegli
...
nella sezione 6.3. Il ritardo di consegna smaschera l'inoltro differito; quello in
...
nella sezione 6.3. Il legame device↔persona resta il limite strutturale, ed è esattamente
...
seriale, e un valore sentinella nel frame finché l'orologio non è valido (sezione 10),
...
(sezione 9.1). All'avvio sincronizza via NTP, o riceve l'ora da seriale; finché l'ora non
...
**Perché si innesta senza attrito.** Lo stack è lo stesso di WeMeet, adottato 1:1:
...
(è il vincolo dichiarato di un notaio-telefono, sezione 9.1), finché un beacon fisso
...
in `firmware/`, sezione 10): canale radio sempre acceso, zero gesti, campionamento più
denso dove il sistema operativo lo concede (Android generoso, iOS opportunistico:
un emettitore continuo non rende continuo il ricevitore, sezione 9.1).
...
l'esatto contrario dell'obiettivo di prodotto (sezione 5). E cosa rivela il confronto:
...
il piano mirato per le righe deboli e i benefici sensibili (sezione 4), non il
...
limite strutturale della sezione 9.1, e va introdotto quando il valore in gioco lo giustifica.
